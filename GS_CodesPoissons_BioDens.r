...
poissons.tableau.brut <- function(save=FALSE) {
  ...
  # Calcul de densitÃ© et de biomasse par observation/espÃ¨ce/transect/campagne
  # Formule de distance sampling ajustÃ©e pour les observations individuelles
  ...
  # DensitÃ©/biomasse par *observation*
  ds.calc$dm.obs <-  0.5*(ds.calc$D1+ds.calc$D2)+0.5 # dm.observation
  ds.calc$dens.obs <- ds.calc$N/(2*ds.calc$dm.obs*LT) # densitÃ©.observation
  ds.calc$bio.obs <- ds.calc$N *ds.calc$a*(ds.calc$L^ds.calc$b)/(2*ds.calc$dm.obs*LT)

  # ** Assemblage ** Tableau donnÃ©es brutes **
  # DensitÃ©/biomasse par observation/espÃ¨ce/transect/annÃ©e/campagne
  tb.1 <- ds.calc[,c("Campagne","An","St","Code_SP","Obs","N",
                     "dens.obs","bio.obs","D1","D2","L","a","b")]

  # Joint Ã :
  # CharactÃ©ristiques du transect:
  # annÃ©e/campagne/transect/plongeur/zone impact/...
  # ... gÃ©omorphologie/pÃªche cpue/pÃªche effort/taux de sÃ©dimentation/...
  ...
  # ... Ã©tat commercial/groupe trophique
  ...
  # garde seulement observations N>0 pour rÃ©duire la taille du fichier .csv

  # Filtrer les espÃ¨ces au besoin
...
##################################################
##################################################
# Tableau prÃ©paratoire pour utilisation par
...
BioDens.sp.poissons <- function() { # formerly BD.by.sp()
  ...
  ds.df$dens <- denom.dm$x/(2*ds.df$dm*LT) # DensitÃ©
  ds.df$allN <- denom.dm$x # nombre individus total espÃ¨ce/site/campagne
...
# sÃ©dentaire (sed), territoriale (ter), mobile (mo), trÃ¨s mobile (tmo)
# cible pÃªche nouvelle-calÃ©donie, ou non
...
poissons.ts1 <- function(AS="A",save=FALSE) {
  ...
  ### Appliquer filtre sur espÃ¨ces
  dfh <- filtreTaxo(dfh, action=taxoF.incl, taxtype=taxoF.utaxo, taxnom=taxoF.nom)

  # Formatter
  dfh$Seen <- 0 + dfh$allN>0 # colonne pour prÃ©sence/absence
  ...
  vtype <- c("mean","sd","richness") # diffÃ©rencie richness vu qu'il n'y pas de moyenne/ET Ã  calculer
  # DÃ©finir les facteurs d'aggrÃ©gation des espÃ¨ces
  vcat <- c("total","Peche","Groupe.Trophique","Groupe.Mobil","fmlabel", "Cible")
  # Conserver seulement les catégories qui ont des représentants (e.g. oter fmlabel si le filtre taxo exclu tous les Pom et Chaet
...
################################################
# Tableau synthÃ¨se 2:
# pour chaque espÃ¨ce, moyenne de densitÃ© pour chaque annÃ©e et totale,
...
poissons.p3 <- function(quel.graph="all",AS="A",save=FALSE) {
    ...
    # produire tableau de donnÃ©es Ã  partir de la fonction poissons.ts1()
    TS1 <- poissons.ts1(AS=AS)

    # filtre annuel par dÃ©faut
...
poissons.cible.graph <- function(save=FALSE) {
  ...
  on.exit(EM())
  ################################
  check.dev.size(ww=9, hh=5.3) # taille fenÃªtre

  # Pour chaque geomorphologie, faire une sÃ©rie de temps (+ filtrÃ©es)
  # dÃ©montrant biomasse/densitÃ©/richesse moyenne avec espÃ¨ces ciblÃ©es/non-ciblÃ©es
  # sur la mÃªme fenÃªtre, divisÃ©e par impact
  gr.lab <- c("Moy.dens","Moy.biomasse","sp.rich")
  ...
  # Filtre sur stations
  wfiltre <- "T_A_poissons"; print(paste("stations filtrÃ©es par",wfiltre))
  ...
  # AggrÃ©gation des valeurs par gÃ©omorphologie/impact
  ...
  # DÃ©finition de la fonction graphique par gÃ©omorphologie:
  graph.geo <- function(wgeo, wtype) {
    ...
    # Fonction graphique par impact/tÃ©moin
...
#######################################
# Tableau index pour catÃ©gories figures
