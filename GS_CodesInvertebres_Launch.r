...
# Time-stamp: <2013-01-08 17:38:01 Laura>
...
Run.INV.biodiv <- function() {
  ...
  dd <- sapply(c("Grp2","S_Grp2","Famille"), function(gt)
...
Run.INV.densite <- function(tabl.seulement = TRUE) {
  ...
  # Densité par station:
  dd <- sapply(c("Grp2","S_Grp2","Famille","Genre","G_Sp"),
               function(tt) sapply(c("A","S","Absent"), function(ff)
