...
prep.analyse <- function() {
 ...
 # créer catégories de substrat pour tableaux synthèses
  ...
  dbio <- dbio[dbio$T %in% c("A","B","C"),]; message(paste("Approx",nTnd,"rangées ôtées vu transect non égal à A, B ou C"))
  ...
  message("\n!! Correction temporaire en attendant que la base INV soit nettoyée: G_Sp. Drupa sp. -> Famille=Muricidae")
  message("!! Correction temporaire en attendant que la base INV soit nettoyée: G_Sp. Aplysia sp. -> Famille=Aplysiidae")
  message("!! Correction temporaire en attendant que la base INV soit nettoyée: G_Sp. Paguritta sp., Ciliopagurus strigatus -> S_Groupe=Decapodes\n")
  ...
  # vérifier s'il y a des doubles entrées dans les informations taxonomiques
