...
\newpage
\hyperlink{reglages 1}{Créez les dossiers \texttt{dossier.R} et
...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    {\hyperlink{nouvdb} {Sauvegarder la nouvelle version du tableau en
        .csv et mettez là dans \texttt{dossier.DB}}};
   ...
   {Dans \mcode, modifiez la variable \texttt{filtre.sur.annees}};
    ...
    \node [blockwide, right of=filtresp, node distance = 5.5cm] (filtrespsec)
    {Dans \mcode, mettre \texttt{filtre.sur.especes = TRUE} et
      modifiez la valeur des variables \texttt{taxoF.action},
      \texttt{taxoF.utaxo}, \texttt{taxoF.nom} \hyperlink{filtre::sp}{(voir section \ref{filtre::sp})}};

    \node [decision, below of=filtresp, node distance=4cm] (lancement)
    {Lancement des routines};
        \node [blockwide, left of=lancement, node distance=5.5cm, yshift=-2cm] (manu)
    {Dans \mcode, spécifiez:\\ \texttt{sorties.INV = FALSE}\\
    \texttt{sorties.poissons = FALSE}\\ \texttt{sorties.INV = FALSE}};
        \node [blockwide, right of=lancement, node distance=5.5cm, yshift=-2cm] (auto)
    {Dans \mcode, spécifiez:\\ \texttt{sorties.INV = TRUE}\\
    \texttt{sorties.poissons = TRUE}\\ \texttt{sorties.INV = TRUE}};
%  \node[block, below of=auto, node distance = 3cm, yshift=5cm](autoinfo)
%  {\hyperlink{filtre::sp}{ben la kessé}};
...
%      mother\_code, options TRUE, source(mother\_code)}
  ...
  %    \path [line] (filtre) -| (update);
    ...
    \path [line] (filtresp) -- node {oui} (filtrespsec) ;
    \path [line,dashed] (filtrespsec) -- (lancement) ;
    \path [line] (lancement) -- node {Manuel} (manu) ;
    \path [line] (lancement) -- node {Automatique} (auto) ;


    \path [line] (filtrean) -- node {non} (filtresp);
%\draw[->,thick,draw=red!50] (filtresp) to [out=270,in=90] (auto);
%    \path [line,dashed] (expert) -- (init);



%    \path [line] (newdata) -- (filtre.an);
%    \path [line] (filtre.sp) -- node {non} (lancement) ;
%    \path [line] (filtre.an) -- node {non} (filtre.sp) ;
%        \path [line] (filtre.sp) -- node {non} (lancement) ;
