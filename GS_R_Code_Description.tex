...
\begin{tikzpicture}[node distance = 2cm, auto]
    ...
    {Lancement des routines};
        \node [block, left of=lancement, node distance=4cm] (manu)
    {Dans mothercode, spécifiez \texttt{sorties.INV = FALSE},
    \texttt{sorties.poissons=FALSE}, \texttt{sorties.INV=FALSE}\\lala};
        \node [block, right of=lancement, node distance=4cm] (auto)
    {Dans mothercode, spécifiez \texttt{sorties.INV = TRUE},
...
\begin{itemize}
 ...
 chaque membre du l'aggrégation taxonomique choisie
  ...
  \item[] \bigtt{inv.RichSpecifique <- function(AS="A", aj.impact=FALSE, aggr="St"):}\\
   ...
   \item[] \hypertarget{i6}{\bigtt{inv.dens.tbl(AS="A", grtax="G\_Sp",
