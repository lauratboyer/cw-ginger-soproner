...
# Time-stamp: <2015-01-20 16:12:54 Laura>

# Fichiers des données de comptage = dpoissons, produit dans prep.analyse()
########################################################
########################################################
# Densité
POIS.dens.gnrl <- function(fspat="St", agtaxo="G_Sp", incl.zero=FALSE) {
# formerly BD.by.sp()
  ...
  # Calcul densité/biomasse par espece/transect/campagne
  # unité de base pour le calcul est "St" par défaut
  ff <- unique(c("Campagne",fspat,"St","T",agtaxo)) # variables d'aggrégation grande -> petite
